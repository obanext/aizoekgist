...
/* ============================================================
   Mobile Panels (filters + results)
   ------------------------------------------------------------
   Functions to open/close the mobile panels. Panels slide in
   from the side on mobile, while on desktop they are always
...
function openFilterPanel(pushHistory = true) {
    ...
    document.getElementById('back-chat-btn').style.display = 'inline-flex';
...
function closeFilterPanel(useHistoryBack = false) {
    ...
    if (!document.getElementById('result-section').classList.contains('open')) {
        ...
        document.getElementById('back-chat-btn').style.display = 'none';
    }
...
/* ============================================================
   History state for browser navigation
============================================================ */
...
function onTouchEnd(e) {
    ...
    } else if (touchActivePanel === 'results' && dx < 0) {
        closeResultPanel(true);
    } else if (touchActivePanel === 'filters' && dx > 0) {
...
/* ============================================================
   Chat flow: threads and messages
============================================================ */
...
function resetThread() {
    ...
    document.getElementById('user-input').placeholder = "What book are you looking for?";
    addOpeningMessage();
    addPlaceholders();
    scrollToBottom();
    resetFilters();
...
async function sendHelpMessage(message) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Help message failed.');
        }
        const data = await response.json();
        hideLoader();
        if (data.response) {
...
/* ============================================================
   Display functions for chat and results
============================================================ */
...
function displayAgendaResults(results) {
    ...
    limitedResults.forEach(result => {
        let formattedDate = result.date || 'No date';
        ...
        const location = result.location || 'No location';
...
async function fetchAndShowDetailPage(ppn) {
    try {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
            ...
            const title = detailJson.record.titles[0] || 'No title';
            const summary = detailJson.record.summaries[0] || 'No summary';
        ...
        displayAssistantMessage('Error fetching detail page.');
    }
}
...
/* ============================================================
   Helpers: loader, scroll, messages
============================================================ */
function showLoader() {
    const messageContainer = document.getElementById('messages');
...
function showErrorMessage() {
    displayAssistantMessage('An error occurred, resetting...');
    hideLoader();
    clearTimeout(timeoutHandle);
    resetThread();
    updateActionButtons();
...
function clearErrorMessage() {
    ...
    if (lastMessage && lastMessage.textContent.includes('error')) {
        messageContainer.removeChild(lastMessage);
    }
}

/* ============================================================
   Send "status klaar" to assistant
============================================================ */
